...
function LiveTenderCard({ onTabChange, onClose }) {
  ...
  const filteredProjects = projectData?.filter((product) => {
    const regStartDate = moment(product.regStartDate);
    const regEndDate = moment(product.regEndDate);
    return currentDate.isBetween(regStartDate, regEndDate, null, '[]');
  });
  ...
  return (
    <Box style={{ padding: "0 16px" }}>
        ...
        <Typography variant="h5">No Live Tender Present. Wait For the Tender To Start.</Typography>
      ) : (
